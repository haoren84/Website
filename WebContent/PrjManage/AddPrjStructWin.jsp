...
<body>
	...
	<script>
		$(function() {
			$('#monitorPointDATA_PROJECT_STRUCTURE_dlg').treegrid({
				url : '../PrjStructResquest?action=GetStructAllMPoint',
	...
	<script>
		...
		function BtnOkFun() {
			...
			if (action == "add") {
				var addRes=AddStructInfo();
				
				var addJson=JSON.parse(addRes);
				
				if(addJson.result=="true"){
					if (window.parent != null && window.parent != undefined) {
						window.parent.CloseAddStruct("add",addJson.errorMsg);
					}
				}else{
					$.messager.alert('提示',addJson.errorMsg);
				}
			}
		}
